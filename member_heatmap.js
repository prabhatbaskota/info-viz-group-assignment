    ...
    function buildHeatmap(containerID, valueField, titleText, interpolator) {
        ...
        [0, 0.5, 1].forEach(t => {
            gradient.append("stop")
                .attr("offset", (t * 100) + "%")
                .attr("stop-color", colorScale(minVal + (maxVal - minVal) * t));
        });

        svg.append("rect").attr("x", width + 20).attr("y", 0).attr("width", legendWidth).attr("height", height).style("fill", `url(#${gradID})`);
        svg.append("text").attr("class", "legend-text").attr("x", width + 40).attr("y", 5).text(maxVal.toFixed(1) + "%");
        svg.append("text").attr("class", "legend-text").attr("x", width + 40).attr("y", height).attr("dominant-baseline", "baseline").text(minVal.toFixed(1) + "%");
        svg.append("text").attr("class", "legend-text").attr("x", width + 30).attr("y", -10).attr("text-anchor", "middle").text("Scale");

        // --- AXES & TITLES ---
        svg.append("g").call(d3.axisLeft(yScale));
        svg.append("g").attr("transform", `translate(0,${height})`).call(d3.axisBottom(xScale));
        const visualCenter = (width + margin.left + margin.right) / 2 - margin.left;
        svg.append("text").attr("class", "chart-title").attr("x", visualCenter).attr("y", -40).attr("text-anchor", "middle").text(titleText);
        svg.append("text").attr("class", "axis-label").attr("transform", "rotate(-90)").attr("y", -margin.left + 35).attr("x", -(height/2)).attr("text-anchor", "middle").text("Age Group");
        svg.append("text").attr("class", "axis-label").attr("x", width/2).attr("y", height + 40).attr("text-anchor", "middle").text("Gender");
    }
